function [S, ExApprox, EyApprox, zAvgApprox, zVarApprox, paramsa, statsa] =...
    ...
    found = zeros(N,1);
    nFoundPairs = 0;
    params1 = [params.mu';params.theta';params.k';params.C';...
    ...
    % then, calculate
    h = abs(params1(pairs(1,1),7) - params1(pairs(1,2),7));
    ...
    for i=1:nFoundPairs
        pair = pairs(i,:);
        [~,ii] = max([params1(pair(1),7),params1(pair(2),7)]);
        a = pair(ii);
        b = pair(pair~=a);
        ...
        betaZ_mid = (params1(pairs(1,1),7) + params1(pairs(1,2),7))/2;
