...
stats_fname_rs = strcat(data_path,...
    'Idealized_Reference_Actuated_4comp_RS_STATISTICS.bin');
stats_rs =...
    read_AIF_four_components_statistics(stats_fname_rs);
params_fname_rs = strcat(data_path,...
    'Idealized_Reference_Actuated_4comp_RS_PARAMETERS.bin');
params_rs =...
...
xlabel('/textbf{Number of seeds}',...
    'Interpreter', 'Latex', 'fontsize', 16)
ylabel({'/textbf{Relative deviation}'; ...
    '/boldmath$(/mathrm{mean} - /mathrm{bound})//mathrm{bound}$'},...
    'Interpreter', 'Latex', 'fontsize', 16)
xlim([2,11])
ylim([-1e-5,9e-5]) 
exportgraphics(gcf, ...
    './Figures/FigSI_idealized_ref_err_4comp.pdf',...
    'ContentType','vector')