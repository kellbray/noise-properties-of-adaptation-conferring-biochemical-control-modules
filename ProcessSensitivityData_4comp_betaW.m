function [S, ExApprox, EyApprox, zAvgApprox, zVarApprox, paramsa, statsa] =...
    ProcessSensitivityData_4comp_betaW(stats, params)
    ...
    % then, calculate
    h = abs(params1(pairs(1,1),2) - params1(pairs(1,2),2));
    ...
    for i=1:nFoundPairs
        pair = pairs(i,:);
        [~,ii] = max([params1(pair(1),2),params1(pair(2),2)]);
        a = pair(ii);
        b = pair(pair~=a);
        ...
        betaW_mid = (params1(pair(1),2) + params1(pair(2),2))/2;
    ...
    S = abs(betaW_mid*der./zAvgApprox); 
end