function [S, ExApprox, EyApprox, zAvgApprox, zVarApprox, paramsa, statsa] =...
    ProcessSensitivityData_4comp_phi(stats, params)
    ...
    % then, calculate
    h = abs(params1(pairs(1,1),1) - params1(pairs(1,2),1));
    ...
    for i=1:nFoundPairs
        pair = pairs(i,:);
        [~,ii] = max([params1(pair(1),1),params1(pair(2),1)]);
        a = pair(ii);
        b = pair(pair~=a);
        ...
        phi_mid = (params1(pair(1),1) + params1(pair(2),1))/2;
